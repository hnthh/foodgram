...
def test_ok(load_ingredients_command_exec):
    ...
    assert Ingredient.objects.filter(name='щавель свежий', measurement_unit='веточка')
    assert Ingredient.objects.filter(name='молоко 3.6%', measurement_unit='г')
    assert Ingredient.objects.filter(name='вишня, протертая с сахаром', measurement_unit='г')
